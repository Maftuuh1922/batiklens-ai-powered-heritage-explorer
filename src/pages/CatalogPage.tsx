...
export function CatalogPage() {
  ...
  const filteredBatiks = batiks.filter(b => {
      ...
      <div className="py-12 md:py-20 lg:py-24">
        ...
        <div className="flex flex-col items-center text-center space-y-8 mb-16 md:mb-24">
          ...
          <motion.h1
            ...
            className="text-6xl md:text-8xl lg:text-9xl font-serif font-bold tracking-tighter monochrome-gradient-text"
          >
            Heritage Index.
          </motion.h1>
          <motion.p
        ...
        {/* Static Filter Bar */}
        <div className="flex flex-col lg:flex-row gap-8 mb-16 items-start lg:items-center justify-between py-8 border-y border-border/40">
            ...
            {categories.map((cat) => (
              <Button
                ...
                className={filter === cat 
                  ? "bg-foreground text-background hover:bg-foreground/90 rounded-full px-6 h-10 text-[10px] font-black uppercase tracking-widest transition-all"
                  : "bg-transparent text-muted-foreground hover:text-foreground border-foreground/10 hover:border-foreground/30 rounded-full px-6 h-10 text-[10px] font-bold uppercase tracking-widest transition-all"
                }
              >
                {cat}
              </Button>
          ...
          <div className="relative w-full lg:w-80 max-w-sm group">
            ...
            <Input
              ...
              className="bg-foreground/5 border-border pl-12 pr-4 h-12 rounded-full font-medium placeholder:text-muted-foreground/50 focus:ring-1 focus:ring-foreground/20 w-full"
        ...
        {filteredBatiks.length > 0 ? (
            ...
            {filteredBatiks.map((batik, idx) => (
              ...
              >
                <Link to={`/batik/${batik.id}`}>
                  <div className="relative aspect-[3/4] rounded-[2rem] overflow-hidden glass-card mb-6 border-border shadow-lg group-hover:shadow-2xl transition-all duration-500">
                    <img
                      src={batik.imageUrl}
                      alt={batik.name}
                      className="w-full h-full object-cover grayscale-[20%] transition-transform duration-1000 group-hover:scale-110"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-background/90 via-background/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 flex flex-col justify-end p-10">
          ...
          <div className="py-20 text-center space-y-4">
            <p className="text-4xl font-serif font-bold opacity-20">No matching motifs found</p>
            <Button variant="link" onClick={() => {setFilter('All'); setSearchQuery('');}} className="text-foreground uppercase tracking-widest text-[10px] font-black">
              Clear all filters
            </Button>
          </div>
        )}
